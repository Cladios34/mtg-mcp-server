...
- Scryfall marks a card `not_legal` in EVERY format until its set's release day, including fully spoiled cards months out. Any format/legality filter therefore silently drops upcoming cards while the response still reads as exhaustive. Every DISCOVERY tool with a `format` parameter reports what its filter hid in `unreleased_excluded` (shared guard `utils/unreleased.py`; the `unreleased` collector on `filter_cards`/`random_card` keeps the probe's criteria identical to the search's by construction). `null` = not checked, `[]` = checked and nothing hidden — never conflate them. An empty result whose filter hid cards must come back as an annotated result, never a bare "not found" error. VALIDATION tools (`deck_validate`, `bulk_format_legality`, `bulk_card_in_formats`, `bulk_ban_list`) deliberately keep answering `not_legal`: that IS their answer, do not "fix" them.
