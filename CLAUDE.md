...
- A keyword arriving from a tool argument is bounded at `MAX_KEYWORD_LENGTH` (64) before anything compiles it. Compiling the cost pattern is linear in the keyword's length and runs on the single event loop: one 400 KB argument froze the server for 3.3s, and the 256-entry cache then held it. The longest real Magic keyword is 23 characters. Any new client-controlled string that feeds a regex, a cache key, or a per-character loop needs the same treatment.
...
- Rules retrieval is lexical, and its ceiling is the wording of the question. `RulesService.keyword_search` matches a single term as a substring and a multi-word query term by term, each weighted by inverse document frequency; `search_terms` is the same engine for callers that parsed their own terms (a whole scenario would trip `MAX_SEARCH_LENGTH`). Measure with `scripts/measure_rules_recall.py` against the 30 annotated questions — never by eye on a case you picked. Current: 17/30 in the top 5, **15/17 for questions naming a mechanic against 2/13 for questions in plain language**. That gap is the whole story: "My 3/3 gets -3/-3, what happens to it?" is answered by 704.5f, which says "toughness 0 or less", a word the question never uses. Only embeddings close that; more lexical tuning will not.
- `deck_rules_map` is the only bridge between the rules engine and a decklist. Two things it teaches. **Subrule selection is not a prefix test**: `702.2` is a prefix of `702.20`-`702.29`, so deathtouch came back with 64 subrules instead of 6 — use `RulesService.subrules`, which requires an alphabetic suffix, and keep `section_rules` for genuine numeric prefixes. **The corpus is dated and incomplete on purpose**: it updates a few times a year, Warp is absent from the April 2025 file entirely, so any tool reading it must name what it could not cover and state the version rather than going quiet.
- Two ranking traps already paid for here, both worth recognising elsewhere. **A rank that means nothing must not be fed into a score**: results were merged by their position within each word's hit list, but that list was in document order, so rule 903.8 came back 76th of 88 for "command zone" purely because Commander sits at the end of the rulebook. **A bonus must be on the same scale as the score it adjusts**: a flat +5.0 for a glossary-cited rule was not a weighting but an override, since reciprocal rank tops out at 0.1. Removing it gained 2 questions.
...
- Scryfall marks a card `not_legal` in EVERY format until its set's release day, including fully spoiled cards months out. Any format/legality filter therefore silently drops upcoming cards while the response still reads as exhaustive. OWNER DEFAULT (2026-08-04, explicit): every DISCOVERY tool with a `format` parameter INCLUDES upcoming cards, marked `[UNRELEASED — releases date]`, listed in `unreleased_included`; `include_unreleased=false` restores the strict list with the removed cards named in `unreleased_excluded`. Shared guard `utils/unreleased.py`; the `unreleased` collector on `filter_cards`/`random_card` keeps the check's criteria identical to the search's by construction. In either field: `null` = not checked, `[]` = checked and nothing found — never conflate them. An empty strict result whose filter hid cards must come back annotated, never as a bare "not found" error. VALIDATION tools (`deck_validate`, `bulk_format_legality`, `bulk_card_in_formats`, `bulk_ban_list`) deliberately keep answering `not_legal`: that IS their answer, do not "fix" them.
- Tool responses are a budget; the 100KB `ResponseLimitingMiddleware` ceiling is a safety net, not the size control. Past it, the middleware replaces the result with truncated text and silently DROPS `structured_content` — no error, the tool just looks broken (params echo only). Slim fields alone don't save you: an unbounded list re-crosses the ceiling (920 almost-included combos measured on one real deck, twice: `deck_audit_bundle` 2026-07-24, `spellbook_find_decklist_combos` 2026-07-29). Any list in `structured_content` needs slim fields AND an explicit cap (top-N by relevance) with the total stated in both outputs — no silent caps. Reproduce size bugs through the orchestrator (middlewares active), never the provider alone, and always with a real-size deck, never a mini-fixture.
