...
- `RulesService.keyword_search` matches a **literal substring**, not a set of words. `"commander damage"` returns zero rules while 903.10a sits in the corpus saying "combat damage by the same commander". A zero here means the phrase is absent, never that the rules are. Measured recall on the 30-question set (`scripts/measure_rules_recall.py`): 9/30 put an expected rule in the top 5, 14/30 never return it at all — 8/17 for questions naming a glossary term against 1/13 for questions phrased in plain language. Rules that do match often rank below the per-term cutoff (903.8 is 76th of 88 for "command zone"), so they never become candidates and no amount of ranking work can reach them. Tune the retrieval layer, not the ranker, and re-measure with that script.
