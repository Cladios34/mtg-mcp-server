...
class TestToolRegistration:
    ...
    async def test_all_71_tools_registered(self, mcp_client: Client):
        """The orchestrator exposes exactly 71 tools."""
        tools = await mcp_client.list_tools()
        tool_names = sorted(t.name for t in tools)
        # 1 ping + 6 scryfall + 4 spellbook + 2 draft + 2 edhrec + 4 moxfield + 3 spicerack
        # + 4 goldfish + 9 bulk + 31 workflows + 5 rules = 71
        # Workflows went 29 -> 31 with hand_probability and simulate_opening_hands (c382cf0);
        # this count was left stale in that delivery and broke the suite.
