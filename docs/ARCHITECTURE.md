...
| Unreleased-card guard | `unreleased_excluded` on every discovery tool with a format filter | Scryfall marks cards not_legal in every format until release day, so a legality filter silently hides spoiled upcoming cards. One shared collector (`utils/unreleased.py`) wired into `filter_cards`/`random_card` — collecting at the legality check itself means the probe's criteria can never drift from the search's. Validation tools excluded on purpose: not_legal is the answer they exist to give |
