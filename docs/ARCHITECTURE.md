...
| Unreleased-card guard | Discovery tools INCLUDE upcoming cards by default, marked `[UNRELEASED]` (`include_unreleased=true`) | Scryfall marks cards not_legal in every format until release day, so a legality filter silently hides spoiled upcoming cards — and a spoiled card missing already cost a real deck analysis (owner decision 2026-08-04). One shared collector (`utils/unreleased.py`) wired into `filter_cards`/`random_card` — collecting at the legality check itself means the check's criteria can never drift from the search's. `include_unreleased=false` restores strict lists with the removed cards named in `unreleased_excluded`. Validation tools excluded on purpose: not_legal is the answer they exist to give |
